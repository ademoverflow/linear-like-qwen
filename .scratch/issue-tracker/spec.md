...
52. As a User, the Team Issues list is grouped by State, so that I can see work at a glance.
...
**Visual prototype (Phase 1, commit on main)** — three throwaway variants of list density + board card anatomy exist on a prototype route with mock data. The maintainer's direction is a hybrid of variants B and C; the spec defaults to **grouped-by-State list (variant C) + rich board cards (variant B)**. If the intended hybrid is the other pairing (card-stack list + minimal cards), that is a one-line change at build time of the list/board tickets. The prototype route is deleted from main once the winner is folded in and the full variant set is captured on a throwaway branch.
