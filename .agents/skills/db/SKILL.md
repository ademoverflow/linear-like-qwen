...
Take the argument given in the user's message (e.g. `$db history`) and run the corresponding command:
...
- For `shell`: this is interactive and cannot run from an agent session. Instead, tell the user to run `make db-shell` themselves. If they need a specific query, offer to run it via: `docker compose exec db psql -U admin -d db -c "<query>"`
